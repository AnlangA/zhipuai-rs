...
impl SessionSink {
    ...
    /// 此事件用于上传音频至缓冲区。
    /// • 当使用ServerVAD模式时，将由模型自动检测人声启停（VAD）并决定何时提交。
    /// • 当使用ClientVAD模式时，需要手动提交音频。上传时可以自行决定音频长度，音频越短响应时间越快，最长可上传30s；
    /// • 音频发送的最高速率为 50QPS，超过后会被限流丢弃，实时音频流推荐按100ms一帧切分，每秒发送 10 帧
    ///
    /// # 参数
    ...
    /// 此事件用于上传视频帧至缓冲区。注意，视频模型需要至少接收到 2 帧 图片来进行感知。推荐按2FPS（每秒 2 帧）的速率发送图片，发送过快可能会导致积压
    ///
    /// # 参数
